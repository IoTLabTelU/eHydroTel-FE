...
class CropCycleApiService {
  ...
  Future<Responses<CropCycleResponseModel>> getCropCyclesForDashboard(String status, bool active, {int page = 1, int limit = 10}) async {
    return await apiClient
        .get<CropCycleResponseModel>(
          Params<CropCycleResponseModel>(
            ...
            queryParameters: {'status': status, 'active': active.toString(), 'page': page.toString(), 'limit': limit.toString()},
          ),
        )
        .then((response) {
  ...
  Future<Responses<bool>> addCropCycle(SessionData sessionData) async {
    return await apiClient
        .post<bool>(Params<bool>(path: EndpointStrings.cropcycle, fromJson: (json) => json as bool, body: sessionData.toJson()))
        .then((response) {
          if (!response.isSuccess) {
  ...
  Future<Responses<CropCycleResponseModel>> searchCropCycles(String query) async {
    return await apiClient
        .get<CropCycleResponseModel>(
          Params<CropCycleResponseModel>(path: '${EndpointStrings.cropcycle}?q=$query', fromJson: (json) => CropCycleResponseModel.fromJson(json)),
        )
        .then((response) {
          if (!response.isSuccess) {
  ...
  Future<Responses<bool>> updateCropCycle(String cropCycleId, EditSessionData sessionData) async {
    return await apiClient
        .patch<bool>(Params<bool>(path: '${EndpointStrings.cropcycle}/$cropCycleId', fromJson: (json) => json as bool, body: sessionData.toJson()))
        .then((response) {
          if (!response.isSuccess) {
  ...
  Future<Responses<bool>> deleteCropCycle(String cropCycleId) async {
    return await apiClient.delete<bool>(Params<bool>(path: '${EndpointStrings.cropcycle}/$cropCycleId', fromJson: (json) => json as bool)).then((
      response,
    ) {
      if (!response.isSuccess) {
  ...
  Future<Responses<bool>> endCropCycle(String cropCycleId) async {
    return await apiClient.post<bool>(Params<bool>(path: '${EndpointStrings.cropcycle}/$cropCycleId/finish', fromJson: (json) => json as bool)).then((
      response,
    ) {
      if (!response.isSuccess) {
